...
AGENDA = [
    ...
    },
    {
        ...
        "prompt": (
            "❗️ **Action:** What are the team’s main objectives for this venture? Please write your top 1-3 goals or hopes for this project."
        )
    },
    {
        "title": "Team Members",
...
if "conversation_state" not in st.session_state:
    st.session_state.conversation_state = "awaiting_team_input"  # or 'meeting_done'

if "clear_input" not in st.session_state:
    st.session_state.clear_input = False

# ---- This flag ensures agenda prompt only appears once per step ----
if "prompted_current_step" not in st.session_state:
    st.session_state.prompted_current_step = -1
...
# Only add agenda prompt ONCE per step!
if (
    st.session_state.conversation_state == "awaiting_team_input"
    and st.session_state.prompted_current_step != st.session_state.step
):
    add_mentor_message(AGENDA[i]["prompt"])
    st.session_state.clear_input = True
    st.session_state.prompted_current_step = st.session_state.step
    st.rerun()
